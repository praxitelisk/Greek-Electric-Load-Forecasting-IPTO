##############################################################
#####xgboost tuning with full features and model selection####
##############################################################
...
#if (!exists("best.xgboost.parameters.full")) {
best.xgboost.parameters.full = list()
best.xgboost.fit.full = list()
best.xgboost.prediction.full = list()
#}
...
for(i in 1:24) {
  ...
  for (etaValue in seq(0.1, 0.5, 0.1)) {
    for(depthValue in seq(2, 5, 1)) {
      for(roundValue in seq(100, 500, 100)) {
        ...
        if( get(paste("min.mape.", i-1, sep="")) > get(paste("mape.xgboost",i-1,sep=".")) ) {
          ...
          best.xgboost.parameters.full[[paste("best.xgboost.param.", i-1, sep="")]] = c(etaValue, depthValue, roundValue, get(paste("mape.xgboost",i-1,sep=".")), get(paste("mae.xgboost",i-1,sep=".")), get(paste("rmse.xgboost",i-1,sep=".")), get(paste("mse.xgboost",i-1,sep=".")))
          names(best.xgboost.parameters.full[[paste("best.xgboost.param.", i-1, sep="")]]) = list("eta", "depth", "round", paste("mape.xgboost",i-1,sep="."), paste("mae.xgboost",i-1,sep="."), paste("rmse.xgboost",i-1,sep="."), paste("mse.xgboost",i-1,sep="."))
          
          
          best.xgboost.fit.full[[paste("fit.xgboost", i-1, sep=".")]] = get(paste("fit.xgboost",i-1, sep="."))
          
          best.xgboost.prediction.full[[paste("prediction.xgboost",i-1,sep=".")]] = get(paste("prediction.xgboost",i-1, sep="."))
...
mape.xgboost.full.ms = list()
mae.xgboost.full.ms = list()
rmse.xgboost.full.ms = list()
mse.xgboost.full.ms = list()
prediction.xgboost.full.ms = list()
fit.xgboost.full.ms = list()
...
for(i in 1:24) {
  ...
  cat("\n\n training after evaluation model: Load.",i-1," with best depth = ", best.xgboost.parameters.full[[paste("best.xgboost.param.", i-1, sep="")]][["depth"]]," eta = ", best.xgboost.parameters.full[[paste("best.xgboost.param.", i-1, sep="")]][["eta"]]," best round =  ", best.xgboost.parameters.full[[paste("best.xgboost.param.", i-1, sep="")]][["round"]], "\n", sep="")
  ...
  fit.xgboost.full.ms[[paste("fit.xgboost",i-1,sep=".")]] = get(paste("fit.xgboost",i-1, sep="."))
  
  prediction.xgboost.full.ms[[paste("prediction.xgboost",i-1,sep=".")]] = get(paste("prediction.xgboost",i-1, sep="."))
  
  mape.xgboost.full.ms[[paste("mape.xgboost",i-1,sep=".")]] = temp.mape
  mae.xgboost.full.ms[[paste("mae.xgboost",i-1,sep=".")]] = temp.mae
  mse.xgboost.full.ms[[paste("mse.xgboost",i-1,sep=".")]] = temp.mse
  rmse.xgboost.full.ms[[paste("rmse.xgboost",i-1,sep=".")]] = temp.rmse  
...
mean.mape.xgboost.full.ms = mean(unlist(mape.xgboost.full.ms))

cat("calculate the mean mae\n")
mean.mae.xgboost.full.ms = mean(unlist(mae.xgboost.full.ms))

cat("calculate the mean mse\n")
mean.mse.xgboost.full.ms = mean(unlist(mse.xgboost.full.ms))

cat("calculate the mean rmse\n")
mean.rmse.xgboost.full.ms = mean(unlist(rmse.xgboost.full.ms))


cat("mean xgboost mape: ", round(mean.mape.xgboost.full.ms,3), "\n")
cat("mean xgboost mae: ", round(mean.mae.xgboost.full.ms,5), "\n")
cat("mean xgboost mse: ", round(mean.mse.xgboost.full.ms,5), "\n")
cat("mean xgboost rmse: ", round(mean.rmse.xgboost.full.ms,5), "\n")
