####################################################################
#####Rules with full features and default parameters################
####################################################################
...
mape.rule.full.def = list()
mae.rule.full.def = list()
rmse.rule.full.def = list()
mse.rule.full.def = list()
prediction.rule.full.def = list()
fit.rule.full.def = list()
...
for(i in 1:24) {
  ...
  fit.rule.full.def[[paste("fit.rule",i-1,sep=".")]] = get(paste("fit.rule",i-1, sep="."))
  
  prediction.rule.full.def[[paste("prediction.rule",i-1,sep=".")]] = get(paste("prediction.rule",i-1, sep="."))
  
  mape.rule.full.def[[paste("mape.rule",i-1,sep=".")]] = temp.mape
  mae.rule.full.def[[paste("mae.rule",i-1,sep=".")]] = temp.mae
  mse.rule.full.def[[paste("mse.rule",i-1,sep=".")]] = temp.mse
  rmse.rule.full.def[[paste("rmse.rule",i-1,sep=".")]] = temp.rmse
...
mean.mape.rule.full.def = mean(unlist(mape.rule.full.def))

cat("calculate the mean mae\n")
mean.mae.rule.full.def = mean(unlist(mae.rule.full.def))

cat("calculate the mean mse\n")
mean.mse.rule.full.def = mean(unlist(mse.rule.full.def))

cat("calculate the mean rmse\n")
mean.rmse.rule.full.def = mean(unlist(rmse.rule.full.def))


cat("mean rule mape: ", round(mean.mape.rule.full.def,3), "\n")
cat("mean rule mae: ", round(mean.mae.rule.full.def,5), "\n")
cat("mean rule mse: ", round(mean.mse.rule.full.def,5), "\n")
cat("mean rule rmse: ", round(mean.rmse.rule.full.def,5), "\n")
