...
temp.mse = 0
temp.rmse = 0
for(i in 1:length(best.svm.parameters.full)) {
  
  temp.mape  = temp.mape + best.svm.parameters.full[[i]][[3]]
  temp.mae = temp.mae + best.svm.parameters.full[[i]][[4]]
  temp.rmse = temp.rmse + best.svm.parameters.full[[i]][[5]]
  temp.mse = temp.mse + best.svm.parameters.full[[i]][[6]]
  
}

#change this variable to mean.mape.nn.full etc. for full.list.of.features
mean.mape.svm.full = temp.mape/length(best.svm.parameters.full)
mean.mae.svm.full = temp.mae/length(best.svm.parameters.full)
mean.rmse.svm.full = temp.rmse/length(best.svm.parameters.full)
mean.mse.svm.full = temp.mse/length(best.svm.parameters.full)

cat("\n****************\n")
cat("mean svm mape: ", round(mean.mape.svm.full,3), "\n")
cat("mean svm mae: ", round(mean.mae.svm.full,5), "\n")
cat("mean svm mse: ", round(mean.mse.svm.full,5), "\n")
cat("mean svm rmse: ", round(mean.rmse.svm.full,5), "\n")
